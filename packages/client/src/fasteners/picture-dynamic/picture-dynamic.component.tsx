...
/**
 * <Picture /> rendering sources depending on:
 *  - Supported formats
 *  - Media queries
 *
 * Assumes that it's running behind a dynamic resizing iamge CDN.
 * In the original form, this was running behind Cloudinary.
 * It has been adapted to run behind a custom Cloudinary implementation.
 *
...
import { MEDIA_QUERY_BREAK_POINTS_MAP } from "../styles/parts/styles.media-queries";
import { CDN_IMAGE_ENDPOINT } from "../../configuration";
...
function createSrcSet(src: string) {
  return `
    ${CDN_IMAGE_ENDPOINT}/${src},
    ${CDN_IMAGE_ENDPOINT}/${appendParam(src, "dpr", 2)} 2x
  `;
}
